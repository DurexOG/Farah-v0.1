...
async function vMute({ member }, target, reason) {
    if (member.user.id === target.user.id) return `!{i} You cannot mute your self`

    const response = await ModUtils.vMuteTarget(member, target, reason);

    if (typeof response === "boolean") return `!{y} ${target.user.username}'s voice is muted in this server`;
    if (response === "MemberPerm") return `!{i} You do not have permission to voice mute ${target.user.username}`;
    if (response === "BotPerm") return `!{i} I do not have permission to voice mute ${target.user.username}`;
    if (response === "NoVoice") return `!{i} ${target.user.username} is not in any voice channel`;
    if (response === "Already") return `!{i} ${target.user.username} is already muted`;
